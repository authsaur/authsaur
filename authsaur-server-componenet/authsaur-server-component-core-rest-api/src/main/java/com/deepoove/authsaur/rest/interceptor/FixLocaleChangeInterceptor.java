...
/**
 * This is {@link FixLocaleChangeInterceptor}.
 *
 * @author Misagh Moayyed
 * @since 6.4.0
 */
@RequiredArgsConstructor
public class FixLocaleChangeInterceptor extends LocaleChangeInterceptor {
    /**
    ...
    public boolean preHandle(final HttpServletRequest request,
        ...
        if (localeProperties.isForceDefaultLocale()) {
            configureLocale(request, response, parseLocaleValue(localeProperties.getDefaultValue()));
            return true;
        }
...
//            val locale = new Locale(newLocale);
            configureLocale(request, response, parseLocaleValue(newLocale));
        }
